...
namespace EngineLayer.ClassicSearch
{
    public class ClassicSearchEngine : MetaMorpheusEngine
    {
            ...
            : base(commonParameters, fileSpecificParameters, nestedIds)
        {
            ...
            // we will generate reverse digestionProduct decoys w/ in the code just below this (and calculate spectral angles for them later).
            ...
            if (Proteins.Any())
            {
                    ...
                    (range, loopState) =>
                {
                    ...
                    for (int i = range.Item1; i < range.Item2; i++)
                    {
                        ...
                        // digest each protein into peptides and search for each digestionProduct in all spectra within precursor mass tolerance
                        foreach (var digestionProduct in Proteins[i].Digest(CommonParameters.DigestionParams, FixedModifications, VariableModifications, SilacLabels, TurnoverLabels))
                        {
                            if (WriteDigestionCounts)
                                DigestionCountDictionary.Increment((digestionProduct.Parent.Accession, digestionProduct.BaseSequence));
                                
                            PeptideWithSetModifications reversedOnTheFlyDecoy = null;

                            if (SpectralLibrary != null && digestionProduct is PeptideWithSetModifications pep)
                            {
                                int[] newAAlocations = new int[digestionProduct.BaseSequence.Length];
                                reversedOnTheFlyDecoy = pep.GetReverseDecoyFromTarget(newAAlocations);
                            }

                            // clear fragments from the last digestionProduct
                            ...
                            foreach (ScanWithIndexAndNotchInfo scan in GetAcceptableScans(digestionProduct.MonoisotopicMass, SearchMode))
                            {
                                ...
                                // check if we've already generated theoretical fragments for this digestionProduct+dissociation type
                                if (peptideTheorProducts.Count == 0)
                                {
                                    digestionProduct.Fragment(dissociationType, CommonParameters.DigestionParams.FragmentationTerminus, peptideTheorProducts);
                                }
                                ...
                                AddPeptideCandidateToPsm(scan, thisScore, digestionProduct, matchedIons);
