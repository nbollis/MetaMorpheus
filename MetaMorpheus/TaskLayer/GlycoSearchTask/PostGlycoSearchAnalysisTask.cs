            ...
            if (_filteredPsms.Count != 0)
            {
                foreach (var psm in _filteredPsms)
                {
                    ...
                    ProteinGroup newProteinGroup = new ProteinGroup(new HashSet<IBioPolymer>(proteinList),
                        new HashSet<IBioPolymerWithSetMods>(new List<IBioPolymerWithSetMods>(psm.BestMatchingBioPolymersWithSetMods.Select(p=> p.SpecificBioPolymer).ToList())), new HashSet<IBioPolymerWithSetMods>());
