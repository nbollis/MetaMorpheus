...
namespace EngineLayer.ModernSearch
{
    public class ModernSearchEngine : MetaMorpheusEngine
    {
        ...
        public ModernSearchEngine(SpectralMatch[] globalPsms, Ms2ScanWithSpecificMass[] listOfSortedms2Scans, List<IBioPolymerWithSetMods> peptideIndex,
            ...
            if (meetsScoreCutoff && scoreImprovement)
            {
                if (PeptideSpectralMatches[scanIndex] == null)
                {
                    if (GlobalVariables.AnalyteType == AnalyteType.Oligo)
                        PeptideSpectralMatches[scanIndex] = new OligoSpectralMatch(peptide, notch, thisScore, scanIndex, scan, CommonParameters, matchedIons);
                    else
