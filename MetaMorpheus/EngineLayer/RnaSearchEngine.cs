    ...
    public class RnaSearchEngine : MetaMorpheusEngine
    {
        ...
        private void AddPeptideCandidateToPsm(ScanWithIndexAndNotchInfo scan, object[] myLocks, double thisScore, OligoWithSetMods oligo, List<MatchedFragmentIon> matchedIons)
        {
            ...
            if (meetsScoreCutoff)
            {
                ...
                {
                    bool scoreImprovement = OligoSpectralMatches[scan.ScanIndex] == null || (thisScore - OligoSpectralMatches[scan.ScanIndex].RunnerUpScore) > -PeptideSpectralMatch.ToleranceForScoreDifferentiation;

                    if (scoreImprovement)
                    {
                        if (OligoSpectralMatches[scan.ScanIndex] == null)
                        {
                            OligoSpectralMatches[scan.ScanIndex] = new OligoSpectralMatch(oligo, scan.Notch, thisScore, scan.ScanIndex, scan.TheScan, CommonParameters, matchedIons, DigestionParameters);
                        }
                        else
                        {
