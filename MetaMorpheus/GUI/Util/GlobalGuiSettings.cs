    ...
    public class GlobalGuiSettings
    {
            ...
            )
        {
            ...
            List<bool> results = new List<bool>
            {
                ...
                (CheckMinimumAllowedIntensityRatio(minimumAllowedIntensityRatio)),
                (CheckWindowWidthThomsons(windowWidthThomsons)),
                (CheckNumberOfWindows(numberOfWindows)),
        ...
        {
            if (string.IsNullOrWhiteSpace(text) || (double.TryParse(text, NumberStyles.Any, CultureInfo.InvariantCulture, out double windowWidthThomsons) && windowWidthThomsons > 0))
            {
                return true;
            }
            else
        ...
        public static bool CheckNumberOfWindows(string text)
        {
            if (string.IsNullOrWhiteSpace(text) || (double.TryParse(text, NumberStyles.Any, CultureInfo.InvariantCulture, out double numberOfWindows) && numberOfWindows > 0))
            {
                return true;
            }
            else
            {
                MessageBox.Show("The value for number of windows must be greater than one. \n You entered " + '"' + text + '"');
                return false;
            }
        }
