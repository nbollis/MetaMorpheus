...
namespace TaskLayer
{
    public class PostSearchAnalysisTask : MetaMorpheusTask
    {
        ...
        private void WritePsmResults()
        {
            ...
            var psmsForPsmResults = FilteredPsms.Filter(Parameters.AllSpectralMatches,
                CommonParameters,
        ...
        private void WritePeptideResults()
        {
            ...
            var peptidesForPeptideResults = FilteredPsms.Filter(Parameters.AllSpectralMatches,
                CommonParameters,
                includeDecoys: Parameters.SearchParameters.WriteDecoys,
                includeContaminants: Parameters.SearchParameters.WriteContaminants,
                includeAmbiguous: true,
        ...
        private void WriteIndividualResults(bool isPeptideLevel)
        {
            Status($"Writing Individual {(isPeptideLevel ? GlobalVariables.AnalyteType.GetUniqueFormLabel() : GlobalVariables.AnalyteType.GetSpectralMatchLabel())} results...",
                Parameters.SearchTaskId);

            var groupedByFile = Parameters.AllSpectralMatches.GroupBy(p => p.FullFilePath).ToList();
            ...
            Parallel.ForEach(groupedByFile, parallelOptions, fileGroup =>
            {
                try
                {
                    WriteIndividualResultsForFile(fileGroup, isPeptideLevel);
                }
                catch (Exception e)
                {
                    EngineCrashed($"WriteIndividualResults ({(isPeptideLevel ? "Peptide" : "PSM")})", e);
                }
            });
        }

        private void WriteIndividualResultsForFile(IGrouping<string, SpectralMatch> fileGroup, bool isPeptideLevel)
        {
            string strippedFileName = Path.GetFileNameWithoutExtension(fileGroup.Key);

            // Create file-specific copies of PSM FDR info to avoid cross-file contamination
            List<SpectralMatch> fileSpecificMatches = fileGroup.ToList();

            if (isPeptideLevel)
            {
                // Deduplicate to peptide level
                fileSpecificMatches = fileSpecificMatches
            ...
            // FDR Analysis is performed again for each file (modifies in-place)
            string analysisType = isPeptideLevel ? "peptide" : "PSM";
            CalculatePsmAndPeptideFdr(fileSpecificMatches, analysisType, doPep: false);

            var itemsToWrite = FilteredPsms.Filter(fileSpecificMatches,
                CommonParameters,
                includeDecoys: Parameters.SearchParameters.WriteDecoys,
                includeContaminants: Parameters.SearchParameters.WriteContaminants,
                includeAmbiguous: true,
                includeHighQValuePsms: Parameters.SearchParameters.WriteHighQValuePsms,
                filterAtPeptideLevel: isPeptideLevel);

            // Write PSM/Peptide results
            string extension = GlobalVariables.AnalyteType.GetSpectralMatchExtension();
            string label = isPeptideLevel
                ? GlobalVariables.AnalyteType.GetUniqueFormLabel()
                : GlobalVariables.AnalyteType.GetSpectralMatchLabel();

            string writtenFile = Path.Combine(Parameters.IndividualResultsOutputFolder,
                $"{strippedFileName}_{label}s.{extension}");
            WritePsmsToTsv(itemsToWrite, writtenFile, writePeptideLevelResults: isPeptideLevel);
            FinishedWritingFile(writtenFile, new List<string> { Parameters.SearchTaskId, "Individual Spectra Files", fileGroup.Key });

            // Write percolator format (PSM only)
            if (!isPeptideLevel)
            {
                string percolatorFile = Path.Combine(Parameters.IndividualResultsOutputFolder,
                    $"{strippedFileName}_{label}sFormattedForPercolator.tab");
                WritePsmsForPercolator(itemsToWrite.FilteredPsmsList, percolatorFile);
                FinishedWritingFile(percolatorFile, new List<string> { Parameters.SearchTaskId, "Individual Spectra Files", fileGroup.Key });
            }

            // Thread-safe dictionary update for results
            lock (ResultsDictionary)
            {
                if (isPeptideLevel)
                {
                    string peptideResultsText = $"{strippedFileName} - Target {label.ToLower()}s with " +
                        itemsToWrite.GetFilterTypeString() + " <= " + Math.Round(itemsToWrite.FilterThreshold, 2) + ": " +
                        itemsToWrite.TargetPsmsAboveThreshold;
                    ResultsDictionary[(strippedFileName, label)] = peptideResultsText;
                }
                else
                {
                    ...
                    string precursorResultsText = $"{strippedFileName} - {GlobalVariables.AnalyteType.GetPrecursorLabel()}: " +
                        Parameters.NumMs2SpectraPerFile[strippedFileName][1];
                    ResultsDictionary[(strippedFileName, GlobalVariables.AnalyteType.GetPrecursorLabel())] = precursorResultsText;

                    string ms2ScanCountResultsText = $"{strippedFileName} - {GlobalVariables.AnalyteType.GetMs2ScanCountLabel()}: " +
                        Parameters.NumMs2SpectraPerFile[strippedFileName][0];
        ...
        #region Spectral Library Methods
