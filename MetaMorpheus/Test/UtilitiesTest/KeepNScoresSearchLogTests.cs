...
[TestFixture]
public class KeepNScoresSearchLogTests
{
    ...
    public void TestAdd()
    {
        var log = new KeepNScoresSearchLog();
    ...
    [Test]
    public void TestAddOrReplace_KeepNone()
    {
        // This should be identical to the KeepTopScoringOnlySearchLog. 
        var log = new KeepNScoresSearchLog(0.001, 1, 0, 0);
        var pwsm = targetPwsm;
    ...
    [Test]
    public void TestAddOrReplace_KeepOne()
    {
        var log = new KeepNScoresSearchLog(0.001, 1, 1, 1);
        var pwsm = targetPwsm;
        var matchedFragmentIons = emptyList;
        ...
        added = log.AddOrReplace(pwsm, 20.0, 0, true, matchedFragmentIons);
        Assert.That(added, Is.True);
        Assert.That(log.Count, Is.EqualTo(3)); // best scoring, one target, one decoy
        Assert.That(log.Score, Is.EqualTo(20.0));
        Assert.That(log.NumberOfBestScoringResults, Is.EqualTo(1));
        Assert.That(log.RunnerUpScore, Is.EqualTo(10.0));
    ...
    [Test]
    public void TestAddOrReplace_KeepMany()
    {
        var log = new KeepNScoresSearchLog(0.001, 1, 8, 8);
        var pwsm = targetPwsm;
        var matchedFragmentIons = emptyList;
    ...
    {
        var log = new KeepNScoresSearchLog(0.1, 0.5, 2, 2);
    ...
    {
        var log = new KeepNScoresSearchLog(0.03, 1, 14, 26);
        var attempt = new SpectralMatchHypothesis(0, targetPwsm, emptyList, 10.0);
    ...
    public void TestTrimProteinMatches()
    {
        ...
        var searchLog = new KeepNScoresSearchLog(0.1, 0.1, 2, 2);
